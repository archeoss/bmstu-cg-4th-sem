...
from PyQt5.QtWidgets import QMessageBox, QMainWindow, QWidget, QColorDialog, QGraphicsScene
...
def visible(p0, p1, p2):
    Pab1 = (p0.x() - p1.x()) * (p2.y() - p1.y())  # x0 * y1
    Pab2 = (p0.y() - p1.y()) * (p2.x() - p1.x())  # y0 * x1
...
def intersection(p1, p2, w1, w2):
    d_1 = (p2.x() - p1.x()) * (w1.y() - w2.y()) - (w1.x() - w2.x()) * (p2.y() - p1.y())  # Dx*Wy - Wx*Dy
    d_2 = (w1.x() - p1.x()) * (w1.y() - w2.y()) - (w1.x() - w2.x()) * (w1.y() - p1.y())  # Px*Wy - Wx*Py
    t = d_2 / d_1
